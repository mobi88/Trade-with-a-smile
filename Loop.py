...
##
dsfm = pd.read_csv(
    "C:/Users/mobi/iCloudDrive/3L68KQB4HG~com~readdle~CommonDocuments/All Materials/5-Master Thesis/untitled/dsfm.csv")
dsfm = dsfm.iloc[4:]
...
for t in range(0, I):
    ...
    qTjHatList = []
    for n in range(0, U):
        o = np.dot(np.atleast_2d(((1 / h[0]) * quartickernel1d((u.u1Points.iloc[n] - x1[t]) / h[0]))).T,
                   np.atleast_2d((1 / h[1]) * (quartickernel1d((u.u2Points.iloc[n] - x2[t]) / h[0]))))
        pTjHatList.append(np.asmatrix(o))

        l = np.multiply(o, Y[t].T)
        qTjHatList.append(np.asmatrix(l))

    new = np.array(list(map((summ), pTjHatList)))
    #     jPTHat.append([])
    jPTHat.append(new)
    new = np.array(list(map(lambda x: x / J[t], (list(map(summ, pTjHatList))))))
    #     pTHat.append([])
    pTHat.append(new)  ## should sum every matrix in u*u dimension and devide it by Jt
    new1 = np.array(list(map(summ, qTjHatList)))
    #     jQTHat.append([])
    jQTHat.append(new1)
    new1 = np.array(list(map(lambda x: x / J[t], (list(map(summ, qTjHatList))))))
    #     qTHat.append([])
    qTHat.append(new1)
##
while stopCriterion >= tol:
    ...
    for t in range(0, I):
        S1 = sum(np.multiply(np.atleast_2d(qTHat[t]).T, mHat[:,1:]))
        #     S2= np.multiply((np.atleast_2d(pTHat1[1]).T),mHat[:,0])
        S2 = sum(np.multiply(np.multiply(np.atleast_2d(pTHat[t]).T, np.atleast_2d(mHat[:, 0]).T), mHat[:, 1:]))
        SList[t] = (S1 - S2) * delta
    ...
    for t in range(0, I):
        ...
        # stopCriterionAll[t] = sum(np.multiply((np.multiply(ZHat[t], mHat) - np.multiply(ZHatOld[t], mHatOld)), (
        #         np.multiply(ZHat[t], mHat) - np.multiply(ZHatOld[t], mHatOld)))).sum() * delta
        stopCriterionAll[t]=sum((np.multiply(ZHat[t], mHat)- np.multiply(ZHatOld[t], mHatOld))**2).sum()
...
## Vector gamma
g = sum((np.multiply(np.atleast_2d(mHat[:, 0]).T, np.multiply(mHat[:, 1:], np.atleast_2d(pHat).T)))) * delta
## Matrix Gamma
...
for t in range(0, I):
    ...
    for l in range(0, L):
        ...
        # f= sp.interpolate.SmoothBivariateSpline(x1[t], x2[t],mHat[:,l])
        m_l[l] = np.array(f(x1[t], x2[t]))
        # m_l[l]= np.array(f(u.u1Points, u.u2Points))
    YTHat = []
    for l in range(0, L):
        YTHat.append([])
        YTHat[l] = np.array(ZHat[t, l] * m_l[l])
    YHat.append([])
...
for n in range(0, len(numeratorRMSE)):
    ...
    while len(numeratorRMSE[n]) < max(
            map(len, numeratorRMSE)):  ##max(list(map(len, list(map(np.transpose, numeratorRMSE))))):
